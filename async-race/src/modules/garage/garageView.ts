...
import {
  Car, GetCarsResponse, MovingCar, WinnerCar,
} from '../types/data';
import './garageView.scss';

export default class GarageView implements View {
  ...
  private turnEngineToStarted = (car: Car) => this.controller.turnEngine(car.id, 'started').then((movCar) => movCar);

  private turnEngineToStopped = (car: Car) => this.controller.turnEngine(car.id, 'stopped').then((movCar) => movCar);
      ...
      paginatedCars.forEach((car) => {
        ...
        newCar.stopButton.addEventListener('click', () => {
          ...
          this.turnEngineToStopped(car).then((movCar) => {
            const duration = Math.round((movCar.distance / movCar.velocity / 1000) * 100) / 100;
            // stops the car but next race automatically puts the cat to the beginning of the road
  ...
  // not fully implemented, only get the standard first winner
  // and delete it from table on car deletion
  private getWinners = () => {
    this.controller.getWinners().then((winners) => {
      ...
      paginatedCars.forEach((car) => {
        ...
        if (raceMode === 'race') {
          ...
          this.turnEngineToStopped(car).then((movCar) => {
            const duration = Math.round((movCar.distance / movCar.velocity / 1000) * 100) / 100;
            // stops the car but next race automatically puts the cat to the beginning of the road
            this.controller.animationHandler(duration, theCarImg, request);
          });
        }
      });
    }).then(() => Promise.all(promiseArray).then(() => carsTimeArray));
  }
